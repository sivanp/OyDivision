...
% Last Modified by GUIDE v2.5 19-Jun-2011 11:20:29
...
maxVal=max(max(double(im)))/(2^16-1);
...
showThreshold(handles);
% im = getimage(handles.axes1);
% [BW] = roipoly(im,circx,circy);
% plot(circx,circy,'y');
% [xi,yi]=ind2sub(size(im),find(BW>0)); %right now I keep all the mask pixels- might be changed to perimeter later.
% %%TODO need to check how to update existing cells...
% 
...
if(length(inds)==1)
    ...
    movie= changeLineageName(lymphid, name, movie);    
end
if(~isempty(movie))
...
% Hint: get(hObject,'Value') returns toggle state of save_btn


% --- Executes on button press in loadCells_btn.
function loadCells_btn_Callback(hObject, eventdata, handles)
% hObject    handle to loadCells_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% --- Executes on button press in loadfile_btn.
% projectDir=get(handles.edit1 ,'String');
% path=sprintf('%s\\movie', projectDir);
[filename,projectDir]= uigetfile('*.mat', 'load movie struct');
...
hold on
im=getImage(handles.axes1);
th= get(handles.thresh_slider,'Value');
if(th==0)
    hold off;
    return;
end
bwthresh=im2bw(im,th);
inds=find(bwthresh==1);
...
if(~isempty(ind)) %lymph allready has this frame marked
    %check if they are close is not-> should issue a warning
    if(overlap(lymph.locations{ind}, [xs,ys])>0.5)
...
if(~isempty(inds))
    msgbox('mom-daughter coupe allready exist');
    return 
end
end
mom=getLymph(momid,movie);
...
% --- Executes on button press in ExctractFluo_btn.
function ExctractFluo_btn_Callback(hObject, eventdata, handles)
% hObject    handle to ExctractFluo_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[filename,projectDir]= uigetfile('*.tif', 'pick fluoresence type file');
...
if(length(mominds)==1)
   ...
   movie= changeLineageName(lymphid, linname, movie)
else
    momid=inputdlg('what is the mother''s id?');
    momid=momid{1};
    if(isempty(momid))
    ...
    momid=str2num(momid);
    momind=find(movie.momDaughTable(mominds,1)==momid);
    movie.momDaughTable(momind,1)=-1;
end
assignin('base','movie',movie);
...
function movie = changeLineageName(lymphid, name, movie)
[lymph,siteind,lymphind]=getLymph(lymphid, movie);
if(isempty(lymph))
    return;
end
...
for i=1:length(predInds)
    ...
    movie= changeLineageName(predId, predName, movie);    
end
